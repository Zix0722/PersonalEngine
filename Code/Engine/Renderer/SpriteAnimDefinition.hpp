...
public:
	...
	bool LoadFromXmlElement(XmlElement const& element, XmlElement const& anim);
	float GetSecondsPerFrame() const;
	int GetNumOfFrameHaveToPlay() const;

private:
	 SpriteDefinition const& GetSpriteDefAtTimeOnce(float seconds) const;
	 SpriteDefinition const& GetSpriteDefAtTimeLoop(float seconds) const;
	 SpriteDefinition const& GetSpriteDefAtTimePingPong(float seconds) const;
private:
